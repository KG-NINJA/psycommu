...
`README.md` explains the Psycommu project-instruction approach. Read `psycommu.md`
or one relevant `examples/` file only when its template topic is involved.
Preserve the user-filled goals, constraints, references and uncertainty labels.
Example app creation, translation or publishing steps are not requests to run
them while maintaining these documents. Use pnpm/Vite only when the requested
target project has that setup or the user requests its creation.
No package manifest, test suite or deployment workflow is tracked. Check changed
Markdown structure and references; do not create an unrelated app for validation.
